...
public class PurchaseItemPanel extends JPanel {
	...
	private JComboBox<String> komboboks;
	// Warehouse model
	...
	private JComponent drawDialogPane() {
		...
		// KOMBOBOKS RUULIB
		komboboks = new JComboBox<String>();
		fillCBox();
	...
	public void fillCBox() {
		...
		for (int i = 0; i < model.getWarehouseTableModel().getRowCount(); i++) {
			...
			komboboks.addItem((String) (model.getWarehouseTableModel()
					.getValueAt(i, 1)));
		}
	}
	...
	public void addItemEventHandler() {
		...
		if (stockItem != null) {
			...
			for (int i = 0; i < model.getCurrentPurchaseTableModel().getRows()
					.size(); i++) {
				String name = model.getCurrentPurchaseTableModel().getRows()
						.get(i).getName();
				if (name.equals(stockItem.getName())) {
					quant1 = quantity
							+ model.getCurrentPurchaseTableModel().getRows()
									.get(i).getQuantity();
					itemInStock = true;
				}
			...
			if (quant1 > q1) {
				...
				JOptionPane.showMessageDialog(panel,
						"Not enough items in warehouse!", "Warning",
						JOptionPane.WARNING_MESSAGE);
			} else {
				model.getCurrentPurchaseTableModel().addItem(
						new SoldItem(stockItem, quantity));
				model.getCurrentPurchaseTableModel().solditems
						.add(new SoldItem(stockItem, quantity));
			}
		}
	}
